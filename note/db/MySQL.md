...
4. 执行器: SQL语句的实际执行组件（判断一下对表有没有执行查询的权限）
...
一组 4 个文件，每个文件的大小是 1GB。如果redo log 设置太小，很快就会被写满，write pos 一直追着 checkpoint。看到的现象就是**磁盘压力很小，但是数据库出现间歇性的性能下跌。**
...
- 唯一索引用不上 change buffer 的优化机制，如果业务可以接受，从性能角度出发建议优先考虑非唯一索引。
...
- 索引合并（**适用于单表查询**同一张表两个条件都有索引；OR索引取并集；AND索引取交集）

- 深翻页使用子查询查询主键；然后join原表获取完整数据。

- force index（观察执行计划，诱导优化器使用合适的索引。）

- 关联查询关联字段要有索引且类型一致；小表驱动大表。
...
- 如果group by并不需要对结果进行排序，可以在 SQL 语句末尾增加 order by null。```select id%10 as m, count(*) as c from t1 group by m order by null;```
...
- 读写分离 （TDDL/MySQL Router/ShardingShpere）
- 分库分表
  - 分库 按照业务拆分
  - 分表 水平拆分和垂直拆分

SQL 的 一个有力补充， NoSQL !=No SQL ，而是 NoSQL = Not Only SQL 。

常见的 NoSQL 方案有如下 4 类。

- K-V 存储：解决关系数据库无法存储数据结构的问题，以 Redis 为代表。

- 文档数据库 ：解决关系 数据库强 s chema 约束的问题，以 MongoDB 为代表。
...
- 强制走主库方案（关键业务读写操作全部指向主机，非关键业务采用读写分离）
